...
module.exports = {
  ...
  async execute(interaction, masterBot) {
    ...
    await interaction.editReply(buildStatusPayload(masterBot, t));
  ...
  async handleComponent(interaction, masterBot) {
    const { t } = i18n(interaction.locale);
    await interaction.update(buildStatusPayload(masterBot, t));
    logger.debug("Status rafraîchi", { user: interaction.user.tag });
  },
};

// ── Builder ───────────────────────────────────────────────────────────────

function buildStatusPayload(masterBot, t) {
